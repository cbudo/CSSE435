...
AndroidAccessory acc(manufacturer, model, "says how it be like it do", versionStr, "https://www.rose-hulman.edu/~drongla", "00001");
...
char charBallColor(int ballColor) {
  switch (ballColor){
    case BALL_NONE:
      return 'N';
    case BALL_BLACK:
...
void customStringCallbackFromAndroid(String customString) {
  ...
  if (customString.equalsIgnoreCase("Q1")) {
    ...
    sprintf(txBuf, "%c%d", charBallColor(ballColor_1), 1);
    standReplyLength = 2;
    ...
    sprintf(txBuf, "%c%d", charBallColor(ballColor_2), 2);
    standReplyLength = 2;
    mainEventFlags |= FLAG_NEED_TO_SEND_STAND_INFO;
  } 
    ...
    sprintf(txBuf, "%c%d", charBallColor(ballColor_3), 3);
    standReplyLength = 2;
    mainEventFlags |= FLAG_NEED_TO_SEND_STAND_INFO;
  } 
    else if (customString.equalsIgnoreCase("QA")) {
    lcd.print("QA");
    ballColor_1 = stand.determineBallColor(LOCATION_1);
    ballColor_2 = stand.determineBallColor(LOCATION_2);
    ballColor_3 = stand.determineBallColor(LOCATION_3);
    sprintf(txBuf, "%c%d,%c%d,%c%d", charBallColor(ballColor_1), 1, charBallColor(ballColor_2), 2,charBallColor(ballColor_3), 3);
    standReplyLength = 8;
    mainEventFlags |= FLAG_NEED_TO_SEND_STAND_INFO;
  }
  else {
...
void loop() {
  ...
  if (acc.isConnected()) {
    ...
    if (mainEventFlags & FLAG_NEED_TO_SEND_STAND_INFO) {
      mainEventFlags &= ~FLAG_NEED_TO_SEND_STAND_INFO;
      acc.write(txBuf, standReplyLength);
    }
  }
